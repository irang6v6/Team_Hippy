...
<body>
	...
	<div class="container">
<div style="float: center; margin-top: 20px; margin-bottom: 20px;">
    <img src="${R}res/images/HippyFrontDoor3.jpg" style="width: 800px; "/>
	</div>
			...
			<tbody>
				...
				<c:forEach var="store" items="${ stores }" begin="0" end="${ s }">
					<tr data-url="store/edit?id=${store.id}&${pagination.queryString}">
						<td style="color:orange;">${ store.id }</td>
						<td>${ store.name }</td>
						<td>${ store.locate }</td>
					...
					<tbody>
						...
						<c:forEach var="article" items="${ articles }"  begin="0" end="${ i }">
							...
							<tr data-url="article/detail?id=${article.id }&pg=1&sz=15&di=0&ti=0&mi=0&pi=0&si=0&od=0&bd=1&st=">
								<td style="color:orange;">${ article.no }</td>
								<td>${ article.title }</td>
					...
					<tbody>
						...
						<c:forEach var="after" items="${ afters }" begin="0" end="${ j }">
							...
							<tr data-url="article/detail?id=${after.id }&pg=1&sz=15&di=0&ti=0&mi=0&pi=0&si=0&od=0&bd=1&st=">
								<td style="color:orange;">${ after.no }</td>
								<td>${ after.title }</td>
